...
export function navigator(node: HTMLElement, config: Config) {
  ...
  function handleKeyPress(ev: KeyboardEvent) {
    ...
    let nextItem = null;
    // if (!items[currentItemIndex] && ev.key === 'ArrowDown') {
    //   nextItem = items[0];
    // } else if (!items[currentItemIndex] && ev.key === 'ArrowUp') {
    //   nextItem = items.at(-1);
    // }
    ...
    // if (items[currentItemIndex] && ev.key === 'ArrowDown') {
    //   findDownElement(items[currentItemIndex], items);
    // }
    // if (items[currentItemIndex] && ev.key === 'ArrowRight') {
    //   findRightElement(items[currentItemIndex], items);
    // }

    // If at first item and more content above, scroll up
    if (scroller && !nextItem && ev.key === 'ArrowUp' && scroller.scrollTop > 0) {
...
class Item {
  ...
  constructor(elements: HTMLElement[], currentIndex: number, scroller: HTMLElement) {
    if (elements[currentIndex]) {
      this.current = {
        ...
        dimens: elements[currentIndex]?.getBoundingClientRect() || {
          top: 0,
          bottom: 0,
          left: 0,
    ...
    this.others = elements
      .filter((a) => a !== elements[currentIndex])
      .map((a) => ({
        element: a,
        dimens: a.getBoundingClientRect(),
      }))
      .filter((a) => this.isInViewport(a.dimens));
  ...
  isInViewport(rect: DOMRect): boolean {
    ...
    if (rect.bottom < clientHeight - buffer) return true;
    if (rect.top > 0 && rect.top < clientHeight + buffer) return true;
