<script lang="ts">
  ...
  import { cards, view } from '../../stores/view';
  ...
  const cardTitle = $cards.items.find((a) => a.id === context.cardId)?.title;

  let hasCards = false;
  let hasPrevCard = false;
  let hasNextCard = false;
  $: {
    hasCards = $cards.items.length > 1;

    const index = $cards.items.findIndex((a) => a.id === $cards.activeCardId);
    hasPrevCard = $view.wrapCards ? true : !!$cards.items[index - 1];
    hasNextCard = $view.wrapCards ? true : !!$cards.items[index + 1];
  }
</script>
